...
namespace ecgsqa
{
  ...
  struct beat_info
  {
    enum { nart = QRS_names::sizeof_QRS_names };
    ...
    void clear() { flags = 0; for (int i = 0; i < nart; ++i) { inds[i] = 0; f[i] = 0; } hr = 0; Lp = 0; Lt = 0; }
    ...
    static string art_name_s(int ind) { static const char* x[nart] = { "P0", "P", "P2", "Q0", "Q", "R", "S", "S2", "T0", "T", "T2" }; return ind >= 0 && ind < nart ? x[ind] : "?"; }
  ...
  struct sleep_info
  {
    ...
    string str_stage() const { if (istage == -1) { return "W"; } if (istage >= 1 && istage <= 4) { return ntocs(istage); } if (istage == 0) { return "R"; } return "?"; }
  ...
  struct ecg_data
  {
    ...
    enum
    {
      db_ECG = 0x1, db_Resp = 0x8, db_EEG = 0x2, db_BP = 0x4,
      dba_beats = 0x10, dba_beats_eval = 0x20, // dba_beats: R positions only; dba_beats_eval = R positions + HR,P,Q,S,T,noise etc. evaluations (beat_info)
      dba_slpst = 0x40,
    ...
    static void append_text(string& s, const string& ann, const char* delim = "; ") { if (ann.empty()) { return; } if (s.empty()) { s = ann; } else { if (delim) { s += delim; } s += ann; } }

    void trim(ecgismp i_start, ecgismp i_end = -1); // i_start will be limited to >=0; negative i_end == all up to end
    template<class T> void shift_keys(ordhs_t<ecgismp, T>& h, ecgismp delta)
    {
      if (delta == 0 || h.n() == 0) { return; }
      s_long i0 = delta > 0 ? h.n() - 1 : 0; s_long i2 = delta > 0 ? -1 : h.n(); s_long di = delta > 0 ? -1 : 1;
      for (s_long i = i0; i != i2; i += di) { ecgismp k = h(i)->k; const T& v = h(i)->v; h[k + delta] = v; h.remove(k); }
    }
  };
  ...
  struct resampler
  {
    ...
    resampler() : gi0src(0), fsrc(100), fdest(100), b_resample(1), b_down(0), rsd(1), rds(1), pecg(0) {}
    resampler& operator=(const resampler& x) { new (this) resampler(x); return *this; }

    resampler(double fsrc_, double fdest_, const basic_string<ecgfvsmp>* pecg_ = 0, ecgismp gi0src_ = 0)
      : gi0src(gi0src_), fsrc(fsrc_), fdest(fdest_), b_resample(fsrc != fdest), b_down(fdest < fsrc), rsd(fsrc/fdest), rds(fdest/fsrc), pecg(pecg_) {}
    ...
    ecgismp gisrc_le(ecgismp gidest, ecgismp dflt = -1) const { double fi = floor(gidest * rsd); ecgismp i = ecgismp(fi); if (i == fi) { return i; } return dflt; }
      // The nearest source sample index right after or exactly at idest.
      //  If the new value is not valid, the default value is returned.
    ecgismp gisrc_ge(ecgismp gidest, ecgismp dflt = -1) const { double fi = ceil(gidest * rsd); ecgismp i = ecgismp(fi); if (i == fi) { return i; } return dflt; }

      // The nearest destination sample index right before or exactly at isrc.
      //  If the new value is not valid, the default value is returned.
    ecgismp gidest_le(ecgismp gisrc, ecgismp dflt = -1) const { double fi = floor(gisrc * rds); ecgismp i = ecgismp(fi); if (i == fi) { return i; } return dflt; }
      // The nearest destination sample index right after or exactly at isrc.
      //  If the new value is not valid, the default value is returned.
    ecgismp gidest_ge(ecgismp gisrc, ecgismp dflt = -1) const { double fi = ceil(gisrc * rds); ecgismp i = ecgismp(fi); if (i == fi) { return i; } return dflt; }

      // Conversion from time (s) into sample index.
    ecgismp gisrc_le_f(double ts, ecgismp dflt = -1) const { double fi = floor(ts * fsrc); ecgismp i = ecgismp(fi); if (i == fi) { return i; } return dflt; }
    ecgismp gisrc_ge_f(double ts, ecgismp dflt = -1) const { double fi = ceil(ts * fsrc); ecgismp i = ecgismp(fi); if (i == fi) { return i; } return dflt; }
    ecgismp gidest_le_f(double ts, ecgismp dflt = -1) const { double fi = floor(ts * fdest); ecgismp i = ecgismp(fi); if (i == fi) { return i; } return dflt; }
    ecgismp gidest_ge_f(double ts, ecgismp dflt = -1) const { double fi = ceil(ts * fdest); ecgismp i = ecgismp(fi); if (i == fi) { return i; } return dflt; }
    ...
    int v_interp(ecgismp gidest, ecgfvsmp* ret_pv = 0, double* ret_pts = 0, bool b_max = false, bool b_strict = false) const;
  ...
  private:
    ...
    struct _sqrsdata
    {
      ...
      ecgfvsmp maxslope, slopecrit, scmax, scmin, f[10]; bool b_f; // maxslope: global max abs filter value
  ...
  public:
    ...
    ecgismp gisrc() const { return r1.b_down ? r1.gisrc_le(_gidest - 1) : r1.gisrc_le(_gidest) - 1; }
    ...
    alg_sqrs(double fsrc_, const basic_string<ecgfvsmp>& ecg_, ordhs_t<ecgismp, beat_info>& beats_, ecgismp gi0src_ = 0)
      : r1(fsrc_, 250, &ecg_, gi0src_), pbeats(&beats_)
      { reset(gi0src_); }

    void reset(ecgismp gi0src_) { r1.gi0src = gi0src_; _gidest = r1.interp_gidest_min(false); _d.reset(); _state = 1; }
  ...
  struct alg_fnd_t
  {
    typedef Val t_val; typedef Ind t_ind;

    alg_fnd_t(t_ind ifeed0_, double S0_, t_ind nimax_, t_ind nw_ = 0)
    {
      _bs = false; _s = 0;
      _i = ifeed0_; _nints = -2; _nN = 0;
      _s0 = S0_;
      _sum = 0; _x1 = 0; _x2 = 0;
      _nim = nimax_; if (_nim < 1) { _nim = 1; }
      _nw = nw_; if (_nw < 0) { _nw = 0; }
    ...
    {
      ++_i;
      if (_nints <= -2) { _x1 = _x2 = x; _nints = -1; return; }
      if (_nints == -1) { _x1 = _x2; _x2 = x; _nints = 0; return; }
      if (_bs) { _bs = false; _nints = 0; _sum = 0; }
      ++_nints; _sum += std::fabs(_x2 - _x1);
      ...
      {
        try {
          bool bn = _bs && _s >= _s0; _w().push(bn); if (bn) { ++_nN; }
          while(t_ind(_w().size())  > _nw) { bn = _w().front(); _w().pop(); if (bn) { --_nN; } }
        } catch (...) {}
      }
    }
    ...
    bool bN() const { return _bs && _s >= _s0; } // is the current slope a noise ? (may be true at slope end only)
    ...
    t_ind nints() const { return _nints; } // <= 0 until 3 samples fed; after: >= 1 (any slope has at least 1 interval)

    double Lw() const { if (_nw) { return double(_nN) / _nw; } return S(); } // current noise level ? (continuous function)
  ...
  private:
    ...
    double _s0;
    double _sum; t_val _x1, _x2; // _x2 - last fed, _x1 - right before _x2
    t_ind _nim, _nw;
  ...
  private:
    ...
    {
      ecgismp time, sign, nslope, qtime, maxtime, ms160, ms200, ms2000;
      ecgfvsmp maxslope, slopecrit, scmax, scmin, f[10]; bool b_f, b_nslope_a; // maxslope: global max abs filter value
...
//      enum { cnahead = 14, cnintsl = 3, cnnw = 2 };
      enum { cnahead = 14, cnintsl = 3, cnnw = 6 }; // all back from ifeed, must cnintsl + 6 <= cnahead
      ...
      ecgismp ndest_ahead() { return cnahead; } // number of samples at SQRS sampling frequency to queue before feeding SQRS
  ...
  public:
    ...
    alg_sqrs_nc(double fsrc_, const basic_string<ecgfvsmp>& ecg_, ordhs_t<ecgismp, beat_info>& beats_, ecgismp gi0src_ = 0)
      : r1(fsrc_, 250, &ecg_, 0), pbeats(&beats_), _nc(0)
      { reset(gi0src_); }

    void reset(ecgismp gi0src_) { r1.gi0src = gi0src_; _gidest = r1.interp_gidest_min(false); _d.reset(); _nc = _nc_control(_gidest); _ncq = std::queue<ecgfvsmp>(); _state = 1; }
  ...
  struct alg_pqrst
  {
    ...
    ecgismp min_gisrc() const { if (_gir_prev < 0) { return _gi; } return _gir_prev; }

    alg_pqrst(ecg_data& dat_, ecgismp gi0src_ = 0)
      : r1(dat_.fd, dat_.fd, &dat_.ecg, gi0src_), ms1000(r1.gisrc_le_f(1)),
        _pecg(&dat_), _gir_prev(-1), _tsr_prev(0), _gi(gi0src_), _wgi0(gi0src_), _ir(0), _indb(0),
        _fLw(0), _avg_hr(_cnL)
      { _pat_reset(); }
    ...
    void reset(ecg_data& dat_, ecgismp gi0src_ = 0)
    {
      ...
      ms1000 = r1.gisrc_le_f(1); _pecg= &dat_;
      _gir_prev = -1; _tsr_prev = 0;
      _gi = gi0src_;
      _w.clear(); _w2.clear(); _wgi0 = gi0src_; _ir = 0;
      _pat_reset();
      _fLw = 0; _avg_hr.clear();
    }
    ...
    alg_pqrst _copy_data(bool b_include_pat = true) const { alg_pqrst x = *this; if (!b_include_pat) { x._pat.clear(); x._pat31 = x._pat39 = -1; } return x; }

    const basic_string<ecgfvsmp>& rw() const { return _w; } // origial signal around single beat, with DC subtracted
    const basic_string<ecgfvsmp>& rw2() const { return _w2; } // w with smoothing and reconstruction
    ...
    s_long ind_beat() const { return _indb; } // the last processed beat pos. in beats collection (NOTE becomes invalid when ECG is trimmed)
    double gts(ecgismp iw) const { return r1.ts_src(iw + _wgi0); } // global time for the given window-related sample index

    ecgismp cnwpat() const { return 2 * ms1000; } // total number of samples in the median pattern (max. data window size)
    ecgismp cirpat() const { return ms1000; } // number of samples in median pattern before R peak (or R peak sample index in the pattern)
    ecgismp cnwpat_r() const { return cnwpat() - cirpat(); } // number of samples in median pattern, starting from R peak and after

    double cthr_sigptn() const { return 0.1; } // threshold abs(signal - pattern) to enable correction
    double cthr_noise() const { return 0.08; } // noise detector threshold
    double cdc_t_ms() const { return 80; } // averaging time constant to calculate DC offset for each signal point
    double cdc_k() const { return 0.3; } // (< 1) total signal reduction during cdc_t_ms(), when passing through DC filter

    double chf_t_ms() const { return 35; } // averaging time constant to remove HF component from signal before PQRST estimation
    double chf_k() const { return 0.1; } // (< 1) total signal reduction during chf_t_ms()
    double chf_ptcorr() const { return 1.35; } // for P, T amplitude to compensate HF filter action (depends on chf t, k and P, T width)

    double cpat_join_t_ms() const { return 20; } // max. duration of normal signal between two pattern-based corrections to regard them as one
    ...
    static void _avg_bidi(basic_string<ecgfvsmp>& w, double m1, double m2);
      ...
      //  See also _avg_bidi.
    static void _qrs_centering(basic_string<ecgfvsmp>& w, ecgismp ir, double k, double t_ms, double fd, basic_string<ecgfvsmp>* pavg = 0);
  private:
    void _pat_reset();
  ...
  struct alg_slpst_L1
  {
    ...
    struct plan_slpst
    {
      ...
      double dts_phd() const { return ts - tphd0s; } // duration (s) of the current dominant phase

      plan_slpst() { clear(); }

      void clear()
        { for (int i = 0; i < nhcig; ++i) { hcig[i].clear(); } sd10 = 0; amssd = 0; wi1 = 0; phe = -2; phd = -2; naf = 0; ts = 0; tphd0s = 0; tu0s = 0; }

        // true if phd equals to any of the given phases.
      bool test_phd(int ph1, int ph2 = -10, int ph3 = -10, int ph4 = -10, int ph5 = -10, int ph6 = -10) const;

        // Sets new phase values and updates time point.
      void set_ph(int ph);

        // Shifts cig history back by 1 element.
      void shift_hcig() { for (int i = 1; i < nhcig; ++i) { hcig[i-1] = hcig[i]; } hcig[nhcig-1].clear(); }
    ...
    struct state_L1 : plan_slpst
    {
      ...
      struct ruleset_coefs : _ruleset_coefs_base
      {
        ruleset_coefs()
        {
          k1 = 3.5; k21 = 2.5; k22 = 1.3; k23 = 0.7; k24 = 1; k25 = 2.5; k26 = 3; k27 = 1.7; k28 = 1; k31 = 0.5; k32 = 2; k33 = 3; k34 = 4; k4 = 0.5; k5 = 1; k52 = 1.5; k53 = 2; k6 = 315; k7 = 0.6; k8 = 200; k9 = 0.8; k10 = 400;
          q31 = 0.3; q32 = 1; q5 = 2; q52 = 1.3; q53 = 1; q7 = 1.7; q9 = 0.7; q10 = 0.6; q11 = 1.2; q12 = 5.7; q13 = 1; q14 = 2; q15 = 0.9; q16 = 0.01; q17 = 0.7; q18 = 1;
          j11 = 0.8; j12 = 1.1; j13 = 0.7; j14 = 2.4; j15 = 3; j31 = 0.75; j32 = 1.1; j4 = 0.4; j41 = 0.7;
        }
        ...
        { static const char* x[] = {
          ...
          "[j11]", "[j12]", "[j13]", "[j14]", "[j15]", "[j31]", "[j32]", "[j4]", "[j41]",
        0 }; return x[i]; }
        double& coef(int i) const { return *(double*)(((char*)this) + i * ((char*)&this->k21 - (char*)&this->k1)); }
      };
        ...
        struct rulestat
        {
          struct history_entry { double ts; string rule_name; int eval_result; string statement; string params; history_entry() : ts(0), eval_result(-1) {} };
          bool b_enabled; state_L1* ppl; vector<history_entry> history;
          void operator()(const char* rule_name, bool b_result) {}
          string res_ruleset() const { return string(); } string res_stats() const { return string(); } string res_history(const string& add_heading = string()) const { return string(); }
          rulestat() : b_enabled(false), ppl(0) {}   void clear() { history.clear(); }
        };
      #endif
      ...
      : pecg(0),
        ...
        _q3wi1(100, 0),
        _q31sd10(90, 60), _q31sdn6(6, 0.1), _q31min(20, 0), _q31amssd(160, 0)
      { clear(); }

      void clear()
      {
        _gi = r1.gi0src; _gicignew = r1.gi0src;
        _q1sd10.clear();
        _q2sdn6.clear(); _q2min.clear(); _q2amssd.clear();
        _q3wi1.clear();
        _q31sd10.clear(); _q31sdn6.clear(); _q31min.clear(); _q31amssd.clear();
        _hr_last = 0;
        _rls.clear(); _rls.ppl = this;
    ...
    void reset(ecg_data& dat_, ecgismp gi0src_ = 0)
    {
      stp.clear(); stp.pecg = &dat_;
      stp.r1 = resampler(dat_.fd, dat_.fd, &dat_.ecg, gi0src_);
      stp._gi = gi0src_; stp._gicignew = gi0src_;
    }
